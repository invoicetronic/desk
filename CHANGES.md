...
- chore: the release workflow now pins the .NET SDK with `actions/setup-dotnet` just like `tests.yml`. The `release` job was publishing the distributed binaries with whatever SDK the runner image happened to ship, which is not the toolchain the tests validated; on the current image that SDK fails to compile `Invoices/Detail.cshtml`.
